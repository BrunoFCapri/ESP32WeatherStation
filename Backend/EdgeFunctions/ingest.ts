...
function toISOWithTZ(date, timeZone) {
  const { year, month, day, hour, minute, second } = getZonedParts(date, timeZone);
 ...
 */ async function insertTemporalDB(ts, temperatura, humedad) {
  ...
  console.debug("[Ingest] Payload line protocol", {
    line
  });
 ...
 */ serve(async (req)=>{
  ...
  try {
    ...
    const { error: resumenUpsertError } = await supabase.from("resumen_dia").upsert({
      fecha: today,
    ...
    const { error: minimoUpdateError } = await supabase.from("resumen_dia").update({
      minimo_temperatura: temperatura
    }).eq("fecha", today).or(`minimo_temperatura.is.null,minimo_temperatura.gt.${temperatura}`);
